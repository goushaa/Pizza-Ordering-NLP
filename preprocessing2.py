...
from config import label_map, entities_label_map, intents_label_map
...
def test_tokens_to_integers(tokens, word_to_int):
    ...
    for token in tokens:
        if token not in word_to_int:
            int_tokens.append(0) #<UNK> mapping
        else:
            int_tokens.append(word_to_int[token])    
    return int_tokens

########################################################################################################################
def preprocess_data(data, type, srcKey, topKey, VOCAB_SIZE, word_to_int = None): # Type: 0-->train // 1-->test
# Process each entry
    ...
    for entry in data:
        ...
        entity_indices = [entities_label_map[entity] for entity in entities]
        intent_indices = [intents_label_map[intent] for intent in intents]
        if (type==0):
...
def preprocess_sentence(sentence, word_to_int):
    cleaned_sentence = process_text(sentence)
    tokens = word_tokenize(cleaned_sentence)
    tokenized_sentence = test_tokens_to_integers(tokens, word_to_int)

    return tokens, tokenized_sentence