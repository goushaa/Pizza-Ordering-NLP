...
def predict_sentence(model, tokens):
    with torch.no_grad():    
...
def get_orders(tokens, predicted_entities, predicted_intents, reverse_label_map):
    all_pizza_orders = []
    all_drink_orders = []
    ...
    for token, entity, intent in zip(tokens, predicted_entities, predicted_intents):
        entity_type = reverse_label_map[int(entity)]
        intent_type = reverse_label_map[int(intent)]
...
def predict(device, model_name, data_size):
    ...
    pred_entities, pred_intents = predict_sentence(model, tokens_tokenized)
    #print(tokens, pred_entities)

    reverse_labels_map = {v: k for k, v in label_map.items()}
    pizza_orders, drink_orders = get_orders(tokens, pred_entities, pred_intents,reverse_labels_map)
